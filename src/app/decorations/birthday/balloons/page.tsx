...
const CardImage = (Card as any).Image;
const CardTitle = (Card as any).Title;

export const metadata = {
  title: "Birthday Balloons Decorations | MaaChao",
  description: "Find the best birthday balloon decorations for parties.",
};

const balloonPackages = Array.from({ length: 8 }, (_, i) => ({
  id: i + 1,
  name: `Birthday Balloon Decoration #${i + 1}`,
...
export default function BirthdayBalloonsPage() {
    ...
    <MainLayout>
      <div className="w-full bg-white py-4 text-sm">
        <div className="container mx-auto px-4 flex items-center text-black">
          ...
          <span className="mx-2">/</span>
          <span className="text-brand-orange font-semibold">Balloons</span>
        </div>
      </div>
      ...
      <section className="container mx-auto px-4 py-4 bg-white">
        <h1 className="text-3xl font-bold mb-2 text-gray-800">Birthday Balloons Decorations</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
          {balloonPackages.map((pkg) => {
            const slug = pkg.name.toLowerCase().replace(/ /g, "-").replace(/[^a-z0-9-]/g, "");
            return (
              <Link key={pkg.id} href={`/decorations/${slug}`} className="block">
                <Card className="transition-shadow duration-300 hover:shadow-md p-0">
                  <CardImage src={pkg.image} alt={pkg.name} height="h-64" />
                  <div className="p-4">
                    <CardTitle className="truncate text-base font-semibold mb-1">{pkg.name}</CardTitle>
