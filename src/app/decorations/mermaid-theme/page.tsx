...
export const metadata = {
  title: "Mermaid Theme Decorations | MaaChao",
  description: "Find the best Mermaid theme party decorations for birthdays and more.",
};

const mermaidPackages = Array.from({ length: 8 }, (_, i) => ({
  id: i + 1,
  name: `Mermaid Theme Decoration Package #${i + 1}`,
  image: "/placeholder.jpg",
  rating: 4.8,
  reviewCount: 115 + i * 11,
  price: 1799 + i * 210,
  discountPercent: 10 + (i % 3) * 5,
  deliveryDate: "Tomorrow",
}));

export default function MermaidDecorationsPage() {
  return (
    <MainLayout>
      <div className="w-full bg-white py-4 text-sm">
        <div className="container mx-auto px-4 flex items-center text-black">
          ...
          <span className="mx-2">/</span>
          <span className="text-brand-orange font-semibold">Mermaid Theme</span>
        </div>
      </div>
      <section className="container mx-auto px-4 py-4 bg-white">
        <h1 className="text-3xl font-bold mb-2 text-gray-800">Mermaid Theme Decorations</h1>
        <p className="text-gray-700 mb-4">{mermaidPackages.length * 100 + 500} reviews</p>
      </section>
      <section className="container mx-auto px-4 py-4 border-t border-b border-gray-200 bg-white">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center">
            ...
            <span className="text-sm text-gray-500">{mermaidPackages.length * 100 + 500} reviews</span>
          </div>
        </div>
      </section>
      <section className="container mx-auto px-4 py-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {mermaidPackages.map((pkg) => (
