...
import { FaStar, FaRegStar, FaStarHalfAlt, FaCheckCircle, FaGamepad } from "react-icons/fa";
...
const gameProducts: GameProduct[] = [
  {
    ...
    rating: 4.7,
    reviews: 32,
    description: "Classic Monopoly board game for family and friends. Rent for your next game night!",
    offer: "25% Off",
    type: "Board Game",
    rentalPeriod: "2 days",
    features: [
      { icon: <FaCheckCircle className="text-pink-500" />, label: "Complete Set" },
      { icon: <FaGamepad className="text-pink-500" />, label: "Family Fun" },
      { icon: <FaCheckCircle className="text-pink-500" />, label: "Clean & Sanitized" },
    ],
    players: "2-6",
    images: [
    ...
    similar: [],
    alsoLike: [],
    customizationOptions: ["Rental Period", "Add Extra Tokens"],
  },
...
export default function GameProductPage() {
  ...
  const product: GameProduct | undefined = gameProducts.find(
  ...
  const tabs = [
    ...
    { title: "Rental Info", content: `Rental Period: ${product.rentalPeriod}. Players: ${product.players}` },
      ...
      <main className="bg-white min-h-screen">
        ...
        <div className="container mx-auto px-4 pt-4 text-sm">
          <div className="flex items-center text-gray-600">
            ...
            <Link href="/rent-games" className="hover:text-pink-600">Rent Games</Link>
            <span className="mx-2">/</span>
            ...
            <div className="flex lg:flex-col gap-2 order-2 lg:order-1 justify-center lg:justify-start">
              {product.images.map((img, idx) => (
                <div key={idx} className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden">
                  <Image src={img} alt="Game thumbnail" width={64} height={64} className="object-cover w-full h-full" />
                </div>
              ))}
            </div>
