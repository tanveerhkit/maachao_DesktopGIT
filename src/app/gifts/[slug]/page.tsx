...
import { FaStar, FaRegStar, FaStarHalfAlt, FaCheckCircle, FaGift } from "react-icons/fa";
...
const gifts: GiftProduct[] = [
  {
    ...
    rating: 4.9,
    reviews: 87,
    description: "A custom mug with your photo or message. Perfect for birthdays, anniversaries, and special occasions.",
    offer: "17% Off",
    delivery: "Tomorrow",
    features: [
      { icon: <FaCheckCircle className="text-pink-500" />, label: "High Quality Print" },
      { icon: <FaGift className="text-pink-500" />, label: "Gift Wrapped" },
      { icon: <FaCheckCircle className="text-pink-500" />, label: "Fast Delivery" },
    ],
    itemsIncluded: ["1 Mug", "Gift Box", "Personalized Print"],
    images: [
    ...
    similar: [],
    alsoLike: [],
    personalizationOptions: ["Add Photo", "Add Name", "Add Message"],
  },
...
export default function GiftProductPage() {
  ...
  const product: GiftProduct | undefined = gifts.find(
    (g) => g.slug === (typeof slug === "string" ? slug : Array.isArray(slug) ? slug[0] : "")
  );
  if (!product) return notFound();

  const [selectedOption, setSelectedOption] = useState(product.personalizationOptions[0]);
  const [activeTab, setActiveTab] = useState(0);
  const tabs = [
    { title: "Product Description", content: product.description },
    { title: "Items Included", content: product.itemsIncluded.join(", ") },
    { title: "Delivery Information", content: "Standard delivery within 2-4 days. Gift wrapping included." },
  ];
      ...
      <main className="bg-white min-h-screen">
        ...
        <section className="container mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="flex lg:flex-col gap-2 order-2 lg:order-1 justify-center lg:justify-start">
              {product.images.map((img, idx) => (
                <div key={idx} className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden">
                  <Image src={img} alt="Gift thumbnail" width={64} height={64} className="object-cover w-full h-full" />
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="font-semibold mb-2">Personalize</div>
            ...
            <div className="flex gap-4 mt-4">
              ...
              <button className="flex-1 px-6 py-3 bg-white border border-orange-400 text-orange-600 rounded-full font-semibold hover:bg-orange-50 transition-colors">Personalize ✏️</button>
